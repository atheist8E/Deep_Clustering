...
def set_args():
	...
	parser.add_argument("--source_path",     							type = str,        default = "../dat/Credit/")
	parser.add_argument("--target_path",        						type = str,        default = "../log/")
	parser.add_argument("--env_file",        							type = str,        default = "Credit.json")
	parser.add_argument("--num_clusters", 								type = int,        default = 2)
	parser.add_argument("--random_seed",        						type = int,        default = 1)
	parser.add_argument("--model",              						type = str,        default = "Credit")
	parser.add_argument("--description",        						type = str,        default = "K Prototypes")
...
if __name__ == "__main__":
	...
	with open(os.path.join(result_path, "Experiment_Description.txt"), "w") as f:
		...
		dataset = pd.read_csv(os.path.join(os.path.join(args.source_path, "kprototypes"), "Credit.csv"))
		print(dataset)
		...
		args.categorical_index = [dataset.columns.get_loc(categorical_feature) for categorical_feature in args.categorical_features]

		################################################## Model Training ##################################################
			
		training_start_time = time.time()
		model = KPrototypes(n_clusters = args.num_clusters, random_state = args.random_seed)
		model.fit(dataset.to_numpy(), categorical = args.categorical_index)
		predictions = model.predict(dataset.to_numpy(), categorical = args.categorical_index)
